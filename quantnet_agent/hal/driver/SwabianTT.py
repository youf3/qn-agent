...
import asyncio
import numpy as np
from quantnet_agent.hal.hwclasses import SignalMeasurement
import time
import yaml
...
class TimeTaggerManager:
    ...
    async def _initialize_device(self):
        ...
        if self.config_file is None and self.device_addr is None:
            log.error("Time tagger does not exist - no configuration provided")
            return

        try:
            # Run the blocking TimeTagger creation in a thread
            loop = asyncio.get_event_loop()

            if self.device_addr:
                log.debug(f"Connecting to TimeTagger at {self.device_addr}...")
                self.Inst = await loop.run_in_executor(None, TimeTagger.createTimeTaggerNetwork, self.device_addr)
                log.info(f"Time tagger connected to {self.device_addr}")
            else:
                log.debug("Connecting to local TimeTagger...")
                self.Inst = await loop.run_in_executor(None, TimeTagger.createTimeTagger)
                log.info("Local time tagger connected")

            # Load configuration if provided
            if self.config_file:
                await self._load_timetagger_config_async(self.config_file)
    ...
    async def _load_timetagger_config_async(self, config_file):
        """Load TimeTagger configuration asynchronously."""
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, self.load_timetagger_config, config_file)
    ...
    def enableTestSignals(self, Chlist: Optional[List[int]] = None):
        ...
        try:
            ...
            log.debug(f"Test signals enabled for channels: {channels}")
        except Exception as e:
    ...
    def disableTestSignals(self, Chlist: Optional[List[int]] = None):
        ...
        try:
            ...
            log.debug(f"Test signals disabled for channels: {channels}")
        except Exception as e:
    ...
    async def performCoincidenceMeasurement(
        ...
        try:
            ...
            log.debug(f"Coincidence measurement completed for {len(channel_pairs)} channel pairs")
            return data
    ...
    async def performSyncMeasurement(self, filenameWrite: str, Chlist: List[int], timeout: Optional[float] = None):
        ...
        try:
            ...
            async def _perform_measurement():
                ...
                log.debug(f"Starting synchronized measurement for {self.DataAcquisitionTime}s")
                ...
                log.debug(f"Data written to {filenameWrite}")
...
class SwabianTT(SignalMeasurement):
    def __init__(self, property, node, mq_broker_host, mq_broker_port):
        self.address = property.get("device")
        self.config_file = property.get("config_file")

    async def connect(self):
        self.TTU = TimeTaggerManager(device_addr=self.address, config_file=self.config_file)
        await self.TTU._initialize_device()

    async def measure(
        self,
        target: MeasurementType,
        channels=None,
        measurement_time: float = 0.01,
        binwidth: float = 1e3,
        n_bins: int = 100,
        printout: bool = False,
        timeout: float = None,
        ...
        """
        Measure with SwabianTT using MeasurementType enum.
        """
        log.debug(f"Measuring {target.value} at {self.address} with channels {channels}")

        if channels is None:
            raise ValueError(f"Channels must be specified for {target.value} measurement.")

        try:
            # Single call to unified measurement function
            if target == MeasurementType.COINCIDENCE:
                if not all(isinstance(pair, (list, tuple)) and len(pair) == 2 for pair in channels):
                    raise ValueError("For COINCIDENCE, channels must be a list of (ch1, ch2) pairs.")
                return await self.TTU.performCoincidenceMeasurement(
                ...
                return await self.TTU.performChannelMeasurement(
                    target, Chlist=channels, measurement_time=measurement_time, printout=printout, timeout=timeout
                )

        except Exception as e:
            log.error(f"Error during {target.value} measurement: {e}")
            raise
...
async def start():
    TT = SwabianTT(
        {"device": "10.0.0.50", "config_file": "quantnet_agent/config/TimeTaggerConfig.yaml"}, None, None, None
    )
    await TT.connect()
    while True:
        try:
            count_list = await TT.measure(
                MeasurementType.COUNT,
                channels=TT.TTU.Chlist,
                measurement_time=1,
                printout=True,
                timeout=10.0,  # Add timeout to prevent hanging
            )

            # Handle the numpy array properly
            if count_list is not None:
                if count_list.ndim > 1:
                    counts_1d = count_list[:, 0]
                else:
                    counts_1d = count_list
                log.info(np.sum(counts_1d))

        except Exception as e:
            log.error(f"Measurement failed: {e}")

        await asyncio.sleep(1)  # Add delay between measurements
